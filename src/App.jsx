...
function scoreTrack(current, candidate) {
  ...
  const harmonyScore = keyScore(toCamelot(current.key), toCamelot(candidate.key));
  rawScore += harmonyScore;

  if (harmonyScore >= 35) reasons.push("même tonalité");
  else if (harmonyScore >= 20) reasons.push("harmonie compatible");
  else if (harmonyScore > 0) reasons.push("harmonie éloignée");

  const genrePoints = genreScore(current.genres, candidate.genres);
  rawScore += genrePoints;
  if (genrePoints > 0) reasons.push("style proche");

  const yearPoints = yearScore(current.year, candidate.year);
  rawScore += yearPoints;
  if (yearPoints >= 5) reasons.push("époque proche");

  const dancePoints = closeScore(current.danceability, candidate.danceability, 7);
  rawScore += dancePoints;
  if (dancePoints >= 5) reasons.push("dance proche");
  ...
  return {
    ...
    reason: reasons.join(" · ") || "compatibilité faible",
  };
}
        ...
        return (
          <span
            ...
            style={{
              ...
              fontWeight: "bold",
              fontSize: 12,
              background: excellent ? "#d4f8d4" : "#efefef",
              color: excellent ? "#117a11" : "#444",
            }}
          >
            {part}
          </span>
...
export default function App() {
  ...
  async function loadKnownTracks() {
    const res = await fetch("http://localhost:3001/api/known-tracks");
  ...
  async function enrichTrack(track) {
    ...
    try {
      const res = await fetch(
        `http://localhost:3001/api/enrich?artist=${encodeURIComponent(
          track.artists?.[0]?.name || ""
        )}&title=${encodeURIComponent(track.name)}&album=${encodeURIComponent(
          track.album?.name || ""
      ...
      if (!data.found) {
        return {
          ...track,
          enriched: false,
          enrichMessage: data.message || "non trouvé",
        };
      }
      ...
      return {
        ...track,
        enriched: false,
        enrichMessage: "erreur enrichissement",
      };
    }
  }
  ...
  try {
    // 1. Recherche locale dans knownTracks.json
    const localRes = await fetch(
      `http://localhost:3001/api/local-search?q=${encodeURIComponent(query)}`
    );
    ...
    // 3. On évite les doublons local / Spotify
    ...
    // 4. On enrichit uniquement les résultats Spotify non déjà locaux
  ...
  function useTrack(track) {
    setCurrent({
      ...
      image: track.album?.images?.[2]?.url || track.image,
  ...
  const suggestions = useMemo(() => {
    ...
    return knownTracks
      ...
      .filter(
        (t) =>
          !(
            String(t.artist).toLowerCase() === String(current.artist).toLowerCase() &&
            String(t.title).toLowerCase() === String(current.title).toLowerCase()
          )
      )
  ...
  return (
    <div style={{ padding: 20, fontFamily: "Arial", maxWidth: 1100 }}>
      ...
      {status && <p style={{ background: "#f3f3f3", padding: 10 }}>{status}</p>}
      ...
      {spotifyResults.length > 0 && (
        <>
          ...
          {spotifyResults.map((track) => (
            ...
            >
              {track.album?.images?.[2]?.url && (
                <img src={track.album.images[2].url} width="50" alt="" />
              )}
              ...
              <div style={{ flex: 1 }}>
                ...
                {track.bpm && track.key ? (
                  <>
                    ...
                    {toCamelot(track.key) || "?"} · Année :{" "}
                    {track.year || track.album?.release_date?.slice(0, 4) || "?"}
                    <br />
                  ...
                  <>Non enrichi · {track.enrichMessage || "hors base/API"}</>
                )}
              </div>

              <button onClick={() => useTrack(track)} disabled={!track.bpm || !track.key}>
                Utiliser
              </button>
            </div>
          ))}
      ...
      {current && (
        <>
          ...
          <div style={{ border: "2px solid black", padding: 12 }}>
            {current.image && <img src={current.image} width="80" alt="" />}
            <p>
              <strong>{current.title}</strong> — {current.artist}
              <br />
              BPM : {current.bpm || "?"} · Key : {current.key || "?"} · Camelot :{" "}
              {toCamelot(current.key) || "?"} · Année : {current.year || "?"}
              <br />
              Style : {formatGenres(current.genres)} · Dance :{" "}
              {formatDance(current.danceability)}
            </p>
          </div>
          ...
          {suggestions.length === 0 && (
            <p>Pas encore de suggestions. Il faut enrichir plus de titres dans le catalogue.</p>
          )}
...
/>}
              ...
              <div style={{ flex: 1 }}>
                ...
                {formatDance(track.danceability)}
                <br />
               {renderReasonTags(track.reason)}
              </div>

              <button onClick={() => useTrack(track)}>Choisir</button>
            </div>
          ))}
        </>
      )}
